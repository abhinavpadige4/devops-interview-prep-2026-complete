#!/bin/bash
# Solution 2: File Permissions and Ownership
# Demonstrate chmod, chown, chgrp, umask, and special permissions (SUID, SGID, sticky bit)

echo "=== Linux File Permissions Exercise Solution ==="
echo
...
echo "1. Test SUID script: ./suid_script.sh (should run as file owner)"
echo "2. Test SGID directory: Create file in sgid_dir and check group ownership"
echo "3. Test sticky directory: In sticky_dir, users can only delete their own files"
echo "4. Verify umask effect: Create files with different umask values"
echo "5. Check permission calculations: Use stat command to verify octal permissions"